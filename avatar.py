...
DIA = VirtualAvatar(
    ...
    motions=[
        ...
        Motion(
            ...
            # 원래 값에서는 손이 반대쪽 소매 속으로 28.7mm 들어가 있었다.
            #
            # 2026-09-18: 사용자가 '왼손은 겹치지 않을 만큼만 올리고 오른손은
            # 겹치지 않을 만큼만 내려라' 고 했다. 어깨·팔 모양은 원래 그대로
            # 두고 손만 움직였다 — 왼손 +5.2cm(오른 위팔 위에 얹힘),
            # 오른손 -2.0cm(왼 위팔 밑으로). 옷 파고듦 왼손 17 → 2.0mm,
            # 오른손 28.7 → 0mm. 조건: 손이 반대쪽 위팔에 닿을 만큼만
            # 가까이(7~8cm), 왼손은 그 위·오른손은 그 아래, 옷에서 1cm 넘게
            # 뜨면 벌점, 왼손은 위로만·오른손은 아래로만.
            #
            # ★ 왼 위팔 y 를 -88 보다 작게 하면 아래팔이 왼 허벅지를 뚫는다
            #   (맨살 충돌 검사가 잡는다). 옷만 보고 풀면 여기에 걸리므로
            #   그 한계를 조건에 넣고 풀어야 한다.
            #
            # 왼 팔꿈치는 그 뒤에 12도 더 접었다(사용자). 아래팔 y -98 → -110.
            # 이어서 '왼손 더 내리고 오른손 더 올려' — 위팔 y 를 왼쪽 +20,
            # 오른쪽 -22 했다(-28 은 오른팔이 가슴을 7.5mm 뚫는다). 손 높이 차 16.5 → 6.1cm, 옷 겹침은 그대로 0mm.
            # 더 가면(+24/-34 부터) 두 손 높이가 다시 같아져 아래팔이 납작하게
            # 겹쳐 보인다 — 사용자가 처음에 지적한 그 모양이다.
            # 더 접으면(-116, -122) 왼손이 어깨 쪽으로 올라가 팔짱이 아니라
            # 가슴을 감싼 모양이 된다.
            ...
            keys=[
                ...
                {"t": 0.55, "bones": {
                    ...
                    "leftUpperArm": [-67.95, -66.9, -13.72],
                    "leftLowerArm": [37.55, -110.03, 0], "leftHand": [7.77, 3.69, 5.78],
                    "rightShoulder": [5.98, 22.0, -14.53],
                    "rightUpperArm": [103.41, 108.16, -159.35],
                    "rightLowerArm": [13.84, 91.21, 0], "rightHand": [10.04, 17.98, 12.04],
